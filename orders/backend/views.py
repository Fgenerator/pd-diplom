...
from .models import Product, Category, ConfirmEmailToken, Shop, ProductInfo, Order, OrderItem, Parameter, \
    ProductParameter, Contact, User
from .serializers import TestProductSerializer, UserSerializer, CategorySerializer, ShopSerializer, \
...
class TestView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        #order = Order.objects.filter(user_id=request.user.id, id=request.data['id']).first()
        ...
        #user = User.objects.filter(shop__product_infos__ordered_items__order=order)
        #print(user)

        #user = User.objects.filter(shop__product_infos__ordered_items__order=order.id).distinct()
        #print(user)
        ...
        order = Order.objects.filter(user_id=request.user.id, id=3).annotate(
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            return Response(TestProductSerializer(product).data)
        else:
...
class PartnerState(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            return JsonResponse({'Status': False, 'Error': 'Нет добавленных магазинов'}, status=403)
            ...
            if request.data['id'].isdigit():
                try:
                    ...
                    if is_updated:
                        ...
                        new_waybill.send(sender=self.__class__, shop_user_id=shop_user[0].id,
